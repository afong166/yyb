...
async def async_prepare_session(device_id: bytes, login_data: bytes, proxy_url: str = ""):
    ...
    try:
        ...
        cred = await _login_on_session(sess, device_id, login_data)
    finally:
        sess.close()
...
async def async_getcode_with_session(access_psk, psk_secret, cred, payload_protobuf: bytes, *,
    ...
    for sk in cred.keys:
        cmd2881 = SC.build_cmd2881(cred.uin, cred.server_id, sk, payload_protobuf)
...
def parse_oauth_response(payload: bytes) -> dict:
    ...
    OauthAuthorizeResp 字段:
      1: BaseResponse(ret=1, errmsg=2)
      2: is_recent_has_auth
      3: is_slient_auth
      4: redirect_url
      5: scope_list (repeated ScopeInfo: scope=1, desc=2, auth_state=3, ext_desc=4, auth_sub_desc=5)
      6: avatar_list (repeated AvatarInfo: id=1, nickname=2, avatarurl=3)

    OauthAuthorizeConfirmResp 字段:
      1: BaseResponse
      2: redirect_url (可能含 code)
    """
    def _utf8(v):
        if v is None:
            return None
    ...
    base = _pb_get_field(payload, 1)
    ret = None; errmsg = None
    if base:
        ret = _pb_get_varint(base, 1)
        errmsg = _utf8(_pb_get_field(base, 2))
    # redirect_url: Authorize 在 field 4，Confirm 在 field 2
    redirect_url = _utf8(_pb_get_field(payload, 4))
    if redirect_url is None:
        redirect_url = _utf8(_pb_get_field(payload, 2))
    is_recent = _pb_get_varint(payload, 2)
    is_slient = _pb_get_varint(payload, 3)
    scope_list = []
    for scope_info in _pb_get_repeated_field(payload, 5):
        scope_list.append({
            "scope": _utf8(_pb_get_field(scope_info, 1)),
            "desc": _utf8(_pb_get_field(scope_info, 2)),
            "auth_state": _pb_get_varint(scope_info, 3),
            "ext_desc": _utf8(_pb_get_field(scope_info, 4)),
            "auth_sub_desc": _utf8(_pb_get_field(scope_info, 5)),
        })
    avatar_list = []
    for av in _pb_get_repeated_field(payload, 6):
        avatar_list.append({
            "id": _utf8(_pb_get_field(av, 1)),
            "nickname": _utf8(_pb_get_field(av, 2)),
            "avatarurl": _utf8(_pb_get_field(av, 3)),
        })
    ok = (ret == 0)
    return {
